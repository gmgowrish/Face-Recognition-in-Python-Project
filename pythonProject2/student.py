...
class Student:
    def __init__(self, root):
        ...
        # first image
        img = Image.open(r'C:\Users\gmg\PycharmProjects\pythonProject2\image\university.jpg')
        img = img.resize((740, 135), Image.ANTIALIAS)
        self.photoimg = ImageTk.PhotoImage(img)

        f_lbl = Label(self.root, image=self.photoimg)
        f_lbl.place(x=0, y=0, width=740, height=135)
        # second image

        img1 = Image.open(r'C:\Users\gmg\PycharmProjects\pythonProject2\image\images (1).jpeg')
        img1 = img1.resize((790, 135), Image.ANTIALIAS)
        self.photoimg1 = ImageTk.PhotoImage(img1)

        f_lbl = Label(self.root, image=self.photoimg1)
        f_lbl.place(x=740, y=0, width=790, height=135)
        # # third image
        #
        # img2 = Image.open(r'C:\Users\gmg\PycharmProjects\pythonProject2\image\upi3.jpeg')
        ...
        title_lbl = Label(bg_img, text="STUDENT MANAGEMENT SYSTEM",
                          font=("times new roman", 35, "bold"), bg="teal", fg="navy")
        title_lbl.place(x=0, y=0, width=1530, height=45)

        main_frame = Frame(bg_img, bd=2)
        main_frame.place(x=10, y=55, width=1510, height=640)
        # left label frame

        left_frame = LabelFrame(main_frame, bd=2, relief=RIDGE, text="Student Details",
                                font=("times new roman", 19, "bold"))
        left_frame.place(x=10, y=10, width=730, height=620)
        ...
        dep_combo = ttk.Combobox(current_course_frame, textvariable=self.var_dep, font=("times new roman", 9, "bold"),
                                 state="readonly", width=20)
        dep_combo["values"] = ("Select Department", "Computer_science", "Commerce", "Diploma", "BSC")
        dep_combo.current(0)
        ...
        course_combo = ttk.Combobox(current_course_frame, textvariable=self.var_course,
                                    font=("times new roman", 9, "bold"), state="readonly",
                                    width=20)
        course_combo["values"] = (
            "Select course", "Bcom", "BBA", "BCA", "Bsc physics", "Bsc math's", "Chemical Engineering",
        ...
        year_combo = ttk.Combobox(current_course_frame, textvariable=self.var_year, font=("times new roman", 9, "bold"),
                                  state="readonly", width=20)
        year_combo["values"] = (
        ...
        class_student_frame = LabelFrame(left_frame, bd=2, relief=RIDGE, text="Class Student information ",
                                         font=("times new roman", 19, "bold"))
        class_student_frame.place(x=5, y=260, width=720, height=320)
        # studentid
        ...
        student_div_combo = ttk.Combobox(class_student_frame, textvariable=self.var_div,
                                         font=("times new roman", 12, "bold"), state="readonly", width=18)
        student_div_combo["values"] = ("A", "B", "C")
        ...
        radionbtn2 = ttk.Radiobutton(class_student_frame, variable=self.var_radio1, text=" No photo sample",
                                     value="No")
        radionbtn2.grid(row=6, column=1)

        # buttons frame
        btn_frame = Frame(class_student_frame, bd=2, relief=RIDGE, bg="white")
        btn_frame.place(x=0, y=199, width=715, height=35)
        # save
        ...
        # take photo sample
        take_photo_btn = Button(btn1_frame, command=self.generate_dataset, text="Take Photo Sample", width=39,
                                font=("times new roman", 12, "bold"),
                                bg="blue",
                                fg="white")
        ...
        # update photo sample
        update_photo_btn = Button(btn1_frame, command=self.generate_dataset, text="Update Photo Sample", width=39, font=("times new roman", 12, "bold"),
                                  bg="blue",
                                  fg="white")
        ...
        Right_frame = LabelFrame(main_frame, bd=2, relief=RIDGE, text="Student Details",
                                 font=("times new roman", 19, "bold"))
        Right_frame.place(x=780, y=10, width=710, height=620)

        img_right = Image.open(r'C:\Users\gmg\PycharmProjects\pythonProject2\image\stu1.jpeg')
        ...
        Search_combo = ttk.Combobox(Search_frame, font=("times new roman", 9, "bold"), state="readonly", width=16)
        ...
        Search_btn = Button(Search_frame, text="Search ", width=16, font=("times new roman", 10, "bold"), bg="blue",
                            fg="white")
        Search_btn.grid(row=0, column=3, padx=4)

        showall_btn = Button(Search_frame, text="Show All", width=16, font=("times new roman", 10, "bold"), bg="blue",
                             fg="white")
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost", user="root", password="gmg70199", database=" face_recogizer")
        my_cursor = conn.cursor()
    ...
    def update_data(self):
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                Update = messagebox.askyesno("Update", "Do you want to take this student details", parent=self.root)
                if Update > 0:
                    conn = mysql.connector.connect(host="localhost", user="root", password="gmg70199",
                                                   database=" face_recogizer")
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        "update student set Dep=%s,course=%s,Year=%s,Semester=%s,Name=%s,Division=%s,Roll=%s,Gender=%s,Dob=%s,` Email`=%s,Phone=%s,` Address`=%s,Teacher=%s,PhotoSample=%s where `Student id`=%s",
                        (
                            self.var_dep.get(),
                            self.var_course.get(),
                            self.var_year.get(),
